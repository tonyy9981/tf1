...
// Đồng bộ dữ liệu với bảng BlackCat
const syncBlackCat = $.data.read("tf_sync_BlackCat", false);
  ...
  return new Promise(resolve => {
    $.http.get({
      ...
      if (err.response.status === 401) {
        ...
        try {
          if (syncBlackCat === true) {
            let qlSessionInfo = await $.BlackCat.read(tfSessionInfoKey, {}, accountId);
            if (qlSessionInfo && qlSessionInfo["x-request-id"] === sessionInfo["x-request-id"] && qlSessionInfo["valid"] === true) {
              await $.BlackCat.write(tfSessionInfoKey, sessionInfo, accountId);
              $.notification.post(`AccountId ${accountId} đồng thời trong bảng BlackCat đã được đặt thành không hợp lệ`);
            }
          }
        } catch (err) {
          $.logger.error(`Ngoại lệ xảy ra khi AccountId ${accountId} được đặt thành không hợp lệ trong bảng BlackCat\n${err}`);
        }
...
async function syncToBlackCat(currentAccountId = "", currentSessionInfo = null) {

  if (syncBlackCat === true) {
    try {

      let silentSync = true; // Đồng bộ im lặng, sẽ không có thông báo nào bật lên cho việc đồng bộ này
      let [qlAllSessions, qlAppIds] = await $.BlackCat.batchRead([tfSessionInfoKey, {}, "", true], [tfAppIdKey, ""]);
      let batchWriteData = [];

      // Đồng bộ Session với bảng điều khiển BlackCat
      if (currentAccountId !== "" && currentSessionInfo !== null) {
        try {
          const qlSessionInfo = qlAllSessions[currentAccountId];
          $.logger.info(`SessionInfo cũ trong bảng BlackCat\n${JSON.stringify(qlSessionInfo)}`);
          if (!qlSessionInfo || qlSessionInfo["x-session-id"] !== currentSessionInfo["x-session-id"] || qlSessionInfo["valid"] === false) {
            $.logger.info(`Các điều kiện đồng bộ được đáp ứng và Session hiện tại đã được đồng bộ với Bảng điều khiển BlackCat.`);
            batchWriteData.push([tfSessionInfoKey, currentSessionInfo, currentAccountId]);
          } else {
            $.logger.info(`Các điều kiện đồng bộ không được đáp ứng và không cần phải đồng bộ Session hiện tại với bảng BlackCat.`);
          }
          ...
          (error) {
          $.logger.error(`Đã xảy ra ngoại lệ khi đồng bộ Session hiện tại với bảng BlackCat.\n${error}`);
        }
      }

      // Đồng bộ AppId với bảng BlackCat
      try {
        let tfAppIds = $.data.read(tfAppIdKey, "").split(";");
        qlAppIds = qlAppIds.split(";").filter(appId => appId !== "");
        // Xác định xem có AppId không tồn tại trên bảng BlackCat hay không
        let newAppIds = tfAppIds.filter(x => x !== "" && !qlAppIds.includes(x));
        if (newAppIds.length > 0) {
          qlAppIds = qlAppIds.concat(tfAppIds);
          qlAppIds = [...new Set(qlAppIds)];
          $.logger.info(`Lần này bạn cần cập nhật AppId của bảng BlackCat\n${qlAppIds.join(";")}`);
          let strAppIds = "";
          ...
          $.logger.info(`Tất cả AppId đã tồn tại trong bảng BlackCat và việc đồng bộ này sẽ không được thực hiện!`);
        }
      } catch (error) {
        $.logger.error(`Ngoại lệ xảy ra khi đồng bộ AppId với bảng BlackCat\n${error}`);
      }

      // Đồng bộ dữ liệu bảng BlackCat thành cục bộ
      try {
        $.logger.debug(`Tất cả SessionInfo trong bảng điều khiển BlackCat\n${JSON.stringify(qlAllSessions)}`);
        // Duyệt qua Session được lưu trữ trong bảng BlackCat
        for (let accountId of Object.keys(qlAllSessions)) {
          ...
          if (accountId !== "magic_session" && qlAllSessions[accountId]["valid"] === true && accountId !== currentAccountId) {
            ...
            if (localSession["x-request-id"] !== qlSession["x-request-id"] || localSession["valid"] === false) {
              ...
              $.logger.info(`Đồng bộ session của bảng BlackCat ${accountId} với phiên bản cục bộ`);
            }
          }
        }
      } catch (error) {
        $.logger.error(`Một ngoại lệ đã xảy ra khi đồng bộ session của bảng điều khiển BlackCat với bộ nhớ cục bộ.\n${JSON.stringify(error)}`);
      }
      // Ghi dữ liệu theo lô
      const result = await $.BlackCat.batchWrite(...batchWriteData);
      if (result === true) {
        if (silentSync === false) {
          $.notification.post(
            `${scriptName} ${currentAccountId}`,
            "",
            `Thông tin của bạn đã được đồng bộ với bảng BlackCat：\n${$.BlackCat.url}\nNếu địa chỉ trên không phải do bạn cấu hình thì thông tin đã bị rò rỉ!\nHãy tắt tập lệnh ngay lập tức và thay đổi mật khẩu của bạn!\nKiểm tra xem cấu hình bảng BlackCat có bị giả mạo hay không!`);
        } else {
          $.logger.info(`Thông tin tương tự đã tồn tại trong bảng BlackCat, vì vậy sẽ không có thông báo nào bật lên về việc đồng bộ này!\nThông tin của bạn đã được đồng bộ với bảng BlackCat：\n${$.BlackCat.url}\nNếu địa chỉ trên không phải do bạn cấu hình thì thông tin đã bị rò rỉ!\nVui lòng tắt tập lệnh và thay đổi mật khẩu của bạn ngay lập tức!\nKiểm tra xem cấu hình bảng BlackCat có bị giả mạo không!`);
        }
      } else {
        $.notification.post(`Không thể đồng bộ thông tin của bạn với bảng BlackCat：\n${$.BlackCat.url}\nVui lòng kiểm tra cấu hình bảng điều khiển BlackCat!`);
      }
    } catch (error) {
      $.logger.error(`Đã xảy ra ngoại lệ khi đồng bộ hóa dữ liệu với bảng BlackCat\n${error}`);
      $.notification.post(`Không thể đồng bộ thông tin của bạn với bảng BlackCat：\n${$.BlackCat.url}\nVui lòng kiểm tra cấu hình bảng điều khiển BlackCat!`);
    }
  }
}
...
async function getTFSessionInfo() {
  try {
    ...
    if (matches && matches.length > 1) {
      ...
      // Đồng bộ hóa dữ liệu với bảng BlackCat
      await syncToBlackCat(accountId, newSessionInfo);
    } else {
